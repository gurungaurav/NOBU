...
import { fetchAndCreateFile } from "../../utils/convertURLtoFile";
...
export default function UpdateBlogs() {
  ...
  const getBlog = async () => {
    try {
      const res = await getSpecificBlogs(blogId);
      console.log(res.data);
      const data = res.data.data;
      setFieldValue("title", data.title);
      setFieldValue("content", data.content);
      setFieldValue("blogTag", data.blog_Tag.tag_name);
      // Fetch and create main picture file
      const mainPictureFile = await fetchAndCreateFile(data.picture);
      setViewFile(data.picture);
      setFieldValue("picture", mainPictureFile);
    } catch (e) {
      navigate(-1);
      console.log(e);
    }
  ...
  const submitBlog = async (formData) => {
    try {
      const res = await updateBlogsUser(id, blogId, formData);
      if (res.data.success) {
